...
bimfile <-read.table("datafiles/swd65.bim", header = FALSE)
...
###############################################
path = paste("swdrecode/chr",q,".recode.geno.txt" ,sep = "")
...
for (i in 4:length(lines)){
  ...
  j3
  write(j3,file=paste("swdrecode/chr",q,".txt", sep=""),append=TRUE)
}
